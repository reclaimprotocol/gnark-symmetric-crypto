...
func (aes *AES256) AddRoundKey(state [16]frontend.Variable, expandedKey [KS_WORDS * 4]frontend.Variable, from int) [16]frontend.Variable {
	var newState [16]frontend.Variable
	for i := 0; i < 4; i++ {
...
func (aes *AES256) AddRoundKey2(state [16]frontend.Variable, expandedKey [KS_WORDS * 4]frontend.Variable, from int) [16]frontend.Variable {
	var newState [16]frontend.Variable
	ctr := 0
	for i := 0; i < 4; i++ {
...
func (aes *AES256) ExpandKey(key [KEY_SIZE_BYTES]frontend.Variable, sbox0 [256]frontend.Variable, RCon [11]frontend.Variable) [KS_WORDS * 4]frontend.Variable {
	...
	for i < (KS_WORDS * 4) {
		...
		if i%KEY_SIZE_BYTES == 0 {
			...
			t0 = aes.VariableXor(t0, RCon[i/KEY_SIZE_BYTES], 8)
		}
